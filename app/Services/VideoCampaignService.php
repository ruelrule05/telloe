...
class VideoCampaignService
{
    ...
    public static function index()
    {
        return response()->json(app('model-cache')->runDisabled(function () {
            return VideoCampaign::with('videoMessages.videos.userVideo', 'videoMessages.videoMessageLikes', 'videoMessages.user')->where('user_id', Auth::user()->id)->with('videoMessages.conversation', function ($conversation) {
    ...
    public static function store(Request $request)
    {
        ...
        $data = $request->only('name', 'title', 'description', 'initial_message', 'service_id', 'link_preview', 'contactTags');
        $data['user_id'] = $authUser->id;
        $data['contact_tags'] = $data['contactTags'];
        unset($data['contactTags']);
        ...
        if ($request->input('contactTags')) {
            $contacts = Contact::where('user_id', $authUser->id)->whereJsonContains('tags', $request->input('contactTags'))->get();
        ...
        return response()->json($videoCampaign->load('videoCampaignVideos.userVideo'));
    }
