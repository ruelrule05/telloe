					...
					<div v-if="$root.videoMessage.initial_message && ($root.videoMessage.initial_message.message || $root.videoMessage.initial_message.source)" class="p-3 bg-gray-50 border-b">
						<div class="flex items-end">
							...
							<div class="align-self-end pl-1">
								<div class="profile-image profile-image-sm" :style="{ backgroundImage: 'url(' + $root.videoMessage.user_profile_image + ')' }">
									<span v-if="!$root.videoMessage.user_profile_image">{{ $root.videoMessage.user_initials }}</span>
								</div>
							</div>
						</div>
					</div>
...
export default {
	...
	created() {
		...
		if (this.$root.auth) {
			...
			let channel = this.$echo.channel(`conversations.${conversation.id}`);
			channel.listen('NewMessageEvent', message => {
				this.getMessageByID(message.message.id);
			});
		}
