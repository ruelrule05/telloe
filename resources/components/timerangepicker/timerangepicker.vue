			...
			<div class="rounded border w-50 mr-1 flex-grow-1">
				<div class="position-relative dropdown d-flex align-items-stretch" ref="start">
					...
					<div class="w-100 position-static">
						<button class="btn btn-white rounded-0 shadow-none border-0 h-100 btn-block" data-toggle="dropdown" data-display="static" :class="{'text-gray': !time_start}">{{ time_start ? time_start.label : 'Set time' }}</button>
						<div class="dropdown-menu w-100 px-1 overflow-auto" ref="dropdown-start">
							...
							<div v-else class="d-flex flex-wrap">
								<div v-for="hour in startHours" class="w-50 p-1">
									<button :id="`start-${hour.time.replace(':', '')}`" class="btn btn-white btn-sm btn-block p-1 border" :class="{'active': time_start == hour}" @click="time_start = hour">
										{{ hour.label }}
									</button>
								</div>
							</div>
			...
			<div class="rounded border w-50 mr-1 flex-grow-1">
				<div class="position-relative dropdown d-flex align-items-stretch" ref="end">
					...
					<div class="w-100 position-static">
						...
						<div class="dropdown-menu w-100 px-1 overflow-auto" ref="dropdown-end">
							...
							<div v-else class="d-flex flex-wrap">
								<div v-for="hour in endHours" class="w-50 p-1">
									<button :id="`end-${hour.time.replace(':', '')}`" class="btn btn-white btn-sm btn-block p-1 border" :class="{'active': time_end == hour}" @click="time_end = hour">
										{{ hour.label }}
									</button>
								</div>
							</div>
