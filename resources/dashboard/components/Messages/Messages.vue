<template>
	<div class="flex flex-col h-full" @dragover.prevent="dragOver = true" @dragleave.prevent="dragOver = false" @drop.prevent="dropFile">
				...
				<div v-for="(grouped_message, index) in grouped_messages" :key="index" class="message-group">
					...
					<template v-else>
						<div class="flex items-end message-body" :class="{ 'outgoing-message text-right flex-row-reverse': grouped_message.outgoing }">
							<div>
								<div v-if="!grouped_message.sender.ip" class="profile-image profile-image-sm" :style="{ backgroundImage: 'url(' + grouped_message.sender.profile_image + ')' }">
		...
		<!-- Message input -->
		<div v-if="isVideoMessage || (conversation.member && !conversation.member.is_pending)" class="border-top p-4 message-input-wrapper relative">
			<form v-if="showPromptForm" @submit.prevent="notify" class="w-full absolute left-0 bottom-0 bg-white p-2 z-10 border-t">
				<div class="text-sm mb-3">Get notified when someone replies to the chat.</div>
				<input type="text" class="text-sm px-2 py-1.5 mb-3 focus:ring-0" placeholder="Name" required v-model="notifyForm.name" />
				<input type="email" class="text-sm px-2 py-1.5 focus:ring-0" placeholder="Email" required v-model="notifyForm.email" />
				<div class="flex items-center mt-3 justify-between">
					<button class="btn btn-sm btn-outline-primary" type="button" @click="showPromptForm = false"><span>Cancel</span></button>
					<button class="btn btn-sm btn-primary" type="submit"><span>Submit</span></button>
				</div>
			</form>
			<!-- Pending files -->
			...
			<div class="flex items-center space-x-2">
				<div v-if="videoReply" class="flex-grow mb-1">
				...
				<div v-if="promptNotify && (!$root.auth || ($root.auth && $root.videoMessage && $root.auth.id != $root.videoMessage.user_id))" class="mb-1">
					<button class="btn hover:bg-gray-100 btn-sm btn-block w-full text-center border flex items-center space-x-1" type="button" @click="showPromptForm = !showPromptForm">
						<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 -ml-2" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
							<path stroke-linecap="round" stroke-linejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg
