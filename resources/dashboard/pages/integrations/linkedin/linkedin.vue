<template>
	<div class="h-100" v-show="ready">
		<div class="content-header border-bottom lg:static fixed w-full bg-white z-20">
			<span class="ml-7 lg:ml-0 mt-2"> Linkedin </span>
			<div class="ml-auto">
				<button type="button" class="btn btn-md btn-primary flex items-center justify-center">
					<span>Filter</span>
				</button>
			</div>
		</div>
								...
								<template v-for="item in paginated('linkedin')">
									<tr class="border-b hover:bg-gray-100" :key="item.id">
										...
										<td class="px-2 text-sm relative hover-trigger">
											...
											<div class="text-sm absolute bg-white py-2 px-3 rounded-lg shadow-lg shadow-black text-black hover-target z-10">Choose Label</div>
										</td> -->
									</tr>
								</template>
							</paginate>
		...
		<Modal ref="labelSettingModal" class="labelSettingModal" size="sm">
			...
			<div v-for="(label, i) in labelList" :key="i" class="mt-4 flex justify-between items-center cursor-pointer" @click="handleSelectedLabelFromModal(label, selectedLabel.id)">
				<p class="rounded-md py-1 px-4 font-semibold text-sm inline tracking-wide uppercase" :style="{ color: label.textColor, backgroundColor: label.bgColor }">
					{{ label.label }}
				</p>
