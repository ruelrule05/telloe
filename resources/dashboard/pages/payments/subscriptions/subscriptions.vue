<template>
	<div class="min-h-screen flex flex-col">
		...
		<div class="hidden overflow-hidden h-100 flex-grow-1 d-flex flex-column">
			<div class="d-flex flex-column h-100">
				<div class="border-bottom bg-white p-3 d-flex align-items-center">
					...
					<div class="ml-auto d-flex align-items-center">
						...
						<div class="d-inline-flex align-items-center mx-2" :data-intro="$root.intros.subscriptions_index.steps[0]" data-step="1">
						...
						<button :data-intro="$root.intros.subscriptions_index.steps[1]" data-step="2" class="btn btn-light shadow-none d-flex align-items-center" type="button" @click="$refs['addModal'].show()">
									...
									<paginate tag="tbody" name="subscriptions" ref="paginate" :list="subscriptions" :per="15">
										<template v-for="subscription in paginated('subscriptions')">
											<tr v-if="!subscription.placeholder" :key="subscription.id" class="cursor-pointer">
												...
												<td class="align-middle" @click="viewSubscription(subscription)">
													<span class="badge text-capitalize position-relative" :class="'badge-' + (subscription.status ? subscription.status : 'draft')">
			...
			<modal ref="addModal" :close-button="false" @hidden="resetSubscriptionForm()">
				<h5 class="font-heading mb-3">Add Subscription</h5>
				<vue-form-validate @submit="createSubscription()" class="d-flex flex-column flex-grow-1">
					<div class="flex-grow-1">
						...
						<div class="form-group">
							...
							<v-date-picker v-model="newSubscriptionForm.date" :popover="{ placement: 'bottom', visibility: 'click' }" :min-date="new Date()">
								<template v-slot="{ inputValue, inputEvents }">
									<input class="form-control cursor-pointer bg-white" readonly v-on="inputEvents" :value="inputValue" placeholder="Select start date" data-required />
								</template>
							</v-date-picker>
						</div>
					...
					<div class="d-flex align-items-center">
						<button type="button" class="btn btn-light shadow-none" :disabled="newSubscriptionForm.loading" data-dismiss="modal">Cancel</button>
			...
			<modal ref="detailsModal" size="modal-lg">
				<template v-if="selectedSubscription">
					<span class="badge text-capitalize position-relative" :class="'badge-' + (selectedSubscription.status ? selectedSubscription.status : 'draft')">
					...
					<div class="d-flex">
						<div class="w-50">
							...
							<strong>{{ selectedSubscription.services.map(s => s.name).join(',') }}</strong>
					...
					<div class="border p-1 rounded mt-2" v-else-if="selectedSubscription.latest_invoice">
						<table class="table table-sm table-borderless mb-0 p-0">
							...
							<tbody class="shadow-none border-top">
								<tr>
									<td class="pt-2">
										<strong>${{ ((selectedSubscription.latest_invoice.amount || 0) / 100).toFixed(2) }}</strong>
									</td>
