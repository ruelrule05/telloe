				...
				<template v-if="infoTab == 'manage_member'">
					...
					<div v-if="selectedMember" class="p-3">
						<div class="text-center info-profile">
							<div class="user-profile-image d-inline-block" ref="profileImage" :style="{ backgroundImage: 'url(' + selectedMember.profile_image + ')' }">
								<span v-if="!selectedMember.profile_image">{{ selectedMember.initials }}</span>
							</div>
							<h4 class="h5 font-heading conversation-title mb-0 rounded">{{ selectedMember.full_name }}</h4>
							...
							<template v-for="service in services">
								<div v-if="service.is_available" :key="service.id" class="d-flex align-items-center mb-2 rounded p-3 bg-light">
									...
									<div class="ml-auto">
										...
										<toggle-switch :class="{ 'd-none': service.is_loading }" active-class="bg-green" :value="selectedMember.services.find(x => x.assigned_service_id == service.id) ? true : false" @input="memberToggleAssignedService($event, service)"></toggle-switch>
									</div>
								</div>
							</template>
						</div>
				...
				<div v-else-if="infoTab == 'add_member'" class="d-flex flex-column overflow-hidden">
					...
					<div class="p-4 overflow-auto flex-grow-1">
						<vue-form-validate @submit="store">
							...
							<div class="d-flex">
								<button class="btn btn-white border" type="button" @click="infoTab = ''">
									Cancel
								</button>
		...
		<modal ref="resendModal" :close-button="false">
			<template v-if="selectedMember">
				...
				<p class="text-center mt-3">
					...
					<strong>{{ selectedMember.member_user.full_name.trim() || selectedMember.member_user.email }}</strong>
					?
					<br />
				</p>
				<div class="d-flex justify-content-end">
					<button class="btn btn-white border text-body" type="button" data-dismiss="modal">
						Cancel
					</button>
