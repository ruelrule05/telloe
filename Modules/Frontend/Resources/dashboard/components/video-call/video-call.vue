													...
													<template v-if="!isFullScreen">
														<span :class="{ 'd-none': isShrinked }">
															<button
																...
																type="button"
																class="btn btn-shrink-toggle btn-white border badge-pill line-height-1 p-1 ml-1"
																@click="
										...
										<div class="d-flex px-2 py-3 flex-wrap">
											<div class="w-50 px-2 mb-3 recorded position-relative" v-for="recorded in recordedData" :key="recorded.timestamp">
												...
												<div class="position-absolute-center w-100 text-center">
													<button class="btn btn-sm btn-white d-inline-flex align-items-center" @click="downloadRecorded(recorded)"><download-icon transform="scale(0.75)"></download-icon> Download</button>
													<button class="btn btn-sm btn-white d-inline-flex align-items-center" :disabled="recorded.sent" @click="sendRecorded(recorded)">{{ recorded.sent ? 'Sent' : 'Send' }} <arrow-right-icon></arrow-right-icon></button>
												</div>
											</div>
										</div>
									</div>
