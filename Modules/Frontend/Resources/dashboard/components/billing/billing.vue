	...
	<div class="py-4 d-flex flex-row w-100 align-items-center h-100 overflow-auto ">
		...
		<modal ref="cancelSubscription" :close-button="false">
			...
			<div class="d-flex mt-4">
				<button type="button" :disabled="loading" class="btn btn-white border" @click="$refs['cancelSubscription'].hide()">Cancel</button>
		...
		<modal :close-button="false" ref="paymentModal" @hidden="selectedPlan = null; paymentLoading = false;">
			<div v-if="selectedPlan">
				<div class="bg-orange-light rounded p-3 selected-plan-summary">
					...
					<h4 class="mb-0 font-weight-normal d-inline" :class="{'text-warning': selectedPlan.name == 'Annual'}"><strong>${{ parseInt(selectedPlan.price) }}</strong></h4><span :class="{'text-warning': selectedPlan.name == 'Annual'}">.{{ selectedPlan.price.split('.')[1] }}</span> / {{ selectedPlan.interval }}
				</div>
