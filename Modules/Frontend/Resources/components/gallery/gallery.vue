	...
	<div id="gallery" class="position-fixed d-flex flex-nowrap flex-column" :class="{'open': open}">
        ...
        <div v-if="file" class="flex-grow-1 flex-1 d-flex text-center overflow-auto">
            ...
            <div class="flex-grow-1 position-relative">
                ...
                <video controls v-else-if="file.type =='video'" :poster="file.preview" :src="file.source" class="position-absolute-center outline-0"></video>
            </div>
        ...
        <div class="p-2 border-top overflow-hidden">
            <div class="overflow-auto w-100 text-center">
                <div class="d-inline-flex mw-100">
                    <div v-for="m in media" class="preview mx-1 cursor-pointer" :class="{'active': (file || {}).id == m.id}" :style="{'background-image': 'url('+m.preview+')'}" @click="$parent.selectedFile = m"></div>
                </div>
            </div>
        </div>
    </div>
