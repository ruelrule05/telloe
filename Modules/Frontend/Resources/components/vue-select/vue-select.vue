<template>
    <div>
        <div class="dropdown" :class="drop" ref="dropdown" :disabled="disabled">
            <button type="button" class="form-control dropdown-toggle text-left d-inline-flex align-items-center" :class="toggle_button_class" data-toggle="dropdown" ref="dropdown-toggle">
                <span v-if="label" class="text-secondary mr-2">{{ label }}</span>
                <template v-if="searchable">
                    <input type="text" spellcheck="false" @focus="inputFocused" v-model="search" class="p-0 form-control shadow-none border-0 outline-0 input-searchable w-100 bg-transparent line-height-1 font-smoothing-auto" :placeholder="select_placeholder" ref="input-searchable" :data-required="required" />
                </template>
                <template v-else>
                ...
                <div class="ml-auto line-height-0">
                    <chevron-down-icon class="ml-2 dropdown-caret" width="8" height="8" transform="scale(3)"></chevron-down-icon>
                </div>
            </button>

            <div class="bg-white dropdown-menu w-100" :class="[dropdown_class]" ref="dropdown-menu">
