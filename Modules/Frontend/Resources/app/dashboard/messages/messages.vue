<template>
	<div class="d-flex h-100">
					...
					<div class="media" @click="setConversation(conversation)">
					  	...
					  	<div class="media-body pl-3">
                            ...
                            <div class="mt-2 d-flex">
                                <div v-if="conversation.members.length > 1" v-for="member in conversation.members" class="user-profile-image user-profile-image-sm ml-1" :style="{backgroundImage: 'url('+member.user.profile_image+')'}">
	                		...
	                		<div class="px-1 flex-1">
                            	<div v-for="message in grouped_message.messages" v-cloak :id="'message-' + message.id" class="message-item">
		                            <div class="text-wrap position-relative message-content" :class="{'p-0 bg-transparent': ['emoji', 'image', 'video'].find((x) => x == message.type)}">
                                        <div class="message-actions position-absolute px-2 d-flex rounded align-items-center dropup">
                                            <div class="action-content position-relative mr-1 line-height-1">
                                                <div v-tooltip.top="'Tags'" data-toggle="dropdown" class="action-button d-flex align-items-center">
                                                    ...
                                                    <bookmark-icon height="20" width="20" class="cursor-pointer">dsds</bookmark-icon>
                                                </div>
		...
		<div v-if="$root.selectedConversation" class="conversation-details text-center p-3 h-100 position-relative" :class="{'open': $root.detailsTab}">
            ...
            <div class="text-left h-100">
                <!-- Profile -->
                <div v-if="$root.detailsTab == 'profile'" class="h-100 overflow-hidden d-flex flex-column ">
                    ...
                    <div class="btn-group btn-group-sm w-100" role="group">
                        <button type="button" class="btn btn-white border py-2" :class="{'active': $root.profileTab == 'overview'}" @click="$root.profileTab = 'overview'"><small class="font-weight-bold d-block">Overview</small></button>
                        <button type="button" class="btn btn-white border py-2" :class="{'active': $root.profileTab == 'files'}" @click="$root.profileTab = 'files'"><small class="font-weight-bold d-block">Files</small></button>
                        <button type="button" class="btn btn-white border py-2" :class="{'active': $root.profileTab == 'notes'}" @click="$root.profileTab = 'notes'"><small class="font-weight-bold d-block">Notes</small></button>
                        <button type="button" class="btn btn-white border py-2" :class="{'active': $root.profileTab == 'history'}" @click="$root.profileTab = 'history'"><small class="font-weight-bold d-block">History</small></button>
                    </div> 

                    <!-- Overview -->
                    <div v-if="$root.profileTab == 'overview'" class="mt-2">
                    ...
                    <!-- Files -->
                    <div v-else-if="$root.profileTab == 'files'" class="mt-2 d-flex flex-column overflow-hidden h-100">
                        <div class="flex-fill">
                    ...
                    <!-- History -->
                    <div v-else-if="$root.profileTab == 'history'" class="mt-2 overflow-y-only h-100 bg-white rounded shadow-sm">
                        <div class="p-3 overflow-y-auto flex-grow-1 h-100">
                            <div v-for="grouped_message in grouped_messages" v-if="grouped_message.messages.find((x) => x.is_history)" class="w-100 message-group">
                                <small class="font-heading font-weight-bold font-size-base line-height-1 message-sender d-block" :class="{'text-right': grouped_message.outgoing}">{{ grouped_message.sender.full_name }}</small>
                                <div class="d-flex align-items-end message-body" :class="{'outgoing-message text-right flex-row-reverse': grouped_message.outgoing}">
                                    <div>
                                        <div class="user-profile-image" :style="{backgroundImage: 'url('+grouped_message.sender.profile_image+')'}">
                                                    ...
                                                    <div class="action-content cursor-pointer line-height-1">
                                                         <div class="action-content cursor-pointer line-height-1">
                                                            <div class="action-button show">
                                                                <history-icon height="20" width="20" class="active" @click.native="markHistory(message)"></history-icon>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                    ...
                    <!-- Notes -->
                    <div v-else-if="$root.profileTab == 'notes'" class="text-left mt-2">
