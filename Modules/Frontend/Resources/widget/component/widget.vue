<template>
	<div v-if="ready" id="widget" v-cloak>
		<button id="button" class="position-fixed btn btn-primary align-items-center" :class="[open ? 'd-none' : 'd-flex']" type="button" @click="open = !open">
			<calendar-icon class="mr-1" fill="white" width="18" height="18"></calendar-icon> {{ $root.profile.widget.button_text }}
		</button>

		<div class="widget position-fixed" :class="{'d-none': !open}">
			<div class="widget-body bg-white rounded d-flex flex-column overflow-hidden w-100 h-100">
